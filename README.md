...
`tools/verify_citations.py` guards the other direction — the files that *cite* the references. `SKILL.md`, `CODING-RULES.md`, the four sub-skills and everything under `examples/` carry a few hundred subclause citations between them, and a wrong one is invisible: the number exists, it just points somewhere else. So it checks that every cited number is a real LRM subclause, that a citation whose subclause the LRM names after an identifier is not sitting in a sentence about a sibling identifier instead, and that a citation backing a `shall` / `should` / `error` names the subclause carrying the rule rather than its parent. Both spellings count: a section sign in the skills, a bare `LRM` prefix in the example sources.
